appControllers.controller('DaeListCtrl', [
    ...
    function ($rootScope, $scope, $q, $state, logger, NgMap, api, user,
        $timeout, device, $cordovaGeolocation, $stateParams, $ionicHistory) {
        ...
        vm.dragEnd = dragEnd;
        vm.getTextZoom = getTextZoom;
        vm.zoomChanged = zoomChanged;

        vm.onIdle = onIdle;
        vm.currentPosition = { lat: 0, lng: 0 };
        ...
        $scope.$on('$ionicView.loaded', function () {
            NativeStorage.getObject('mapState', function (state) {
                vm.mapState = state;
                if (vm.mapState) {
                    NativeStorage.remove('mapState', function (obj) { });
        ...
        $scope.$watch('vm.selectedIndex', function (current, old) {
            ...
            if (angular.isDefined(current)) {
                ...
                if (current === 0) {
                    ...
                    vm.daeList = [];
                    checkCurrentLocation().then(function (location) {
                        if (location) {
                        ...
                        if (vm.event.currentPosition && vm.map) {
                            ...
                            device.alert("Fix GPS in corso o connessione dati / GPS non attivi. Verifica e riprova", "balanced").then(function () {
        ...
        function getTextZoom() {
            return device.textZoom;
        }

        function errorPosirtion() {
        ...
        function checkCurrentLocation() {
            var q = $q.defer();
            var posOptions = {
                // attendo al massimo 10 secondi
                timeout: 10000,
                // richiedo una posizione precisa usando il gps
                enableHighAccuracy: true,
                // accetto posizioni valide vecchie di 180 secondi
                maximumAge: 600000
            };

            device.checkIsLocationAvailable().then(function (status) {
                if (status) {
                    $cordovaGeolocation.getCurrentPosition(posOptions).then(function (position) {
                        ...
                        NgMap.getMap('customMap').then(function (map) {
                            vm.map = map;
                            q.resolve(location);
                        }, function (error) {
                            logger.info(error);
                    ...
                    q.reject("Fix GPS in corso o connessione dati / GPS non attivi. Verifica e riprova");
                }
            }, function (error) {
                q.reject(error);
            });
        ...
        function getDAEForMap() {
            ...
            if (bounds) {
                ...
                api.searchDAEByGeoJSON(geoJSON).then(function (data) {
                    if (data.length > 0) {
                        var daeMap = [];
                    ...
                    device.hideLoader();
                    if (error.status != 401) {
                        device.alert("Servizio momentaneamente non disponibile", "balanced").then(function () {
                            $state.go("app.home");
                        });
                    }
                    logger.info(error);
                ...
                //se non ho i bounds
                device.hideLoader();
                // stando alla documentazione capita quando la mappa 
                // viene caricata prima che il div venga visualizzato
                ...
                if (center) {
                    ...
                    centerMap()
                    //   vm.map.setCenter(vm.currentPosition);
                }
            }
        }
        ...
        function getDAEForList() {
            var pageSize = 6;
            api.searchDAEByFilter(vm.currentPosition, pageSize, firstIndex).then(function (data) {
                device.hideLoader();
                var allDae = data;
                var n = 0;
        ...
        function goToState(state, dae) {
            if (dae) {
                ...
                NativeStorage.putObject('mapState', mapState, function (obj) { });
        ...
        function centerMap() {
            ...
            checkCurrentLocation().then(function (location) {
                ...
                device.hideLoader();
                device.alert("Fix GPS in corso o connessione dati / GPS non attivi. Verifica e riprova", "balanced").then(
                    function () {
                        $ionicHistory.nextViewOptions({
                            disableBack: true
                        });
