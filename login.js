appControllers.controller('LoginCtrl', [
    ...
    'PushWoosh',
    '$cordovaNetwork',
    function ($scope, $rootScope, logger, device, $translate, $state, api,
        user, $templateCache, $ionicViewSwitcher, $timeout, $mdDialog,
        $cordovaDevice, $ionicHistory, privacy, $q, pushwoosh, $cordovaNetwork) {
                    ...
                    pushwoosh.register(function (result) {
                        ...
                        if (result.type == 'registration') {
                            ...
                            if (result.id) {
                                ...
                                NativeStorage.putString('registrationId', result.id, function (obj) { });
                                q.resolve();
                            } else {
        ...
        function login() {
            ...
            if (vm.loginForm.$valid) {
                ...
                device.showLoader();
                pushwooshRegistration().then(function () {
                    api.login(vm.credentials).then(function (loginResponse) {
                        if (!loginResponse.error) {
                            ...
                            privacy.checkPrivacy().then(function () {
                                //Setto l'autologin per default
                                user.setUserCredentials(vm.credentials, true);
                                api.getLoggedFirstResponderDetails().then(function (profile) {
                                    logger.info(profile);
